...
* Development version
- When you use =embark-prefix-help-command= as your =prefix-help-command=
  and also have the =which-key= package on, the help key now invokes
  Embark even while the =which-key= popup is showing, instead of
  =which-key='s own paging menu. This also works when =which-key= was
  summoned explicitly (e.g. via =which-key-show-top-level= or
  =which-key-show-keymap=), in which case the keymap =which-key= is
...
- New =embark-auto-prefix-help-mode=, a global minor mode similar
  to =which-key-mode= but that prompts you for a command or key binding
