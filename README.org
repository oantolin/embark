...
When =embark-act= quits the minibuffer it uses the =embark-quit= command
to do so, this command quits the minibuffer but keeps the window
configuration intact. Normally pressing =C-g= in the minibuffer restores
...
=find-file-other-window=. (This is not specific to Embark, if you use
...
you may want to bind =embark-quit= to =C-g= in =minibuffer-local-map=.
