...
*** Acting on targets
 ...
 a sequence of file actions without loosing your place in the file
...
*** Switching to a different command without losing what you've typed
 ...
 bindings for command you are likely to want a command to become. The
 way this works is that there is a variable =embark-become-keymaps=
 containing a list of names of keymaps. When you use =embark-become= it
 activates the keymaps in that list which contain the current command.
 For example, the default value of =embark-become-keymaps= contains a
 keymap =embark-become-file+buffer-map= with bindings for several
 commands related to files and buffers, in particular, it binds
 =find-file= to =f=. So when I accidentally try to switch to a buffer for
 a file I haven't opened yet, I can type =C-S-a B f= to switch to
 =find-file=.
