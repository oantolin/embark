...
In addition to those Embark functions, the default values of
=embark-pre-action-hooks= and =embark-post-action-hooks=, use some
built-in Emacs functions that you may want to consider for some
actions of your own:

- =deactivate-mark= :: Unmark the active region. Use this for commands
  you want to act on the region contents but without the region being
  active. The default configuration uses this function as a pre-action
  hook for =occur= and =query-replace=, for example, so that you can use
  them as actions with region targets to search the whole buffer for
  the text contained in the region. Without this pre-action hook using
  =occur= as an action for a region target would be pointless: it would
  search for the the region contents /in the region/, (typically, due to
  the details of regexps) finding only one match!

- =xref-push-marker-stack= :: Push the current location on the xref
