...
wokr with all candidates defined in the current context. For example,
...
If you call =embark-toggle-select= with a prefix argument, instead of
toggling the current target's selection status, it will empty the
selection. So, for example, if you bind =embark-act= to =C-.=, then =C-.
C-u SPC= clears the selection.
